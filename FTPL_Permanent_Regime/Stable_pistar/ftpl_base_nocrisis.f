...
c 8010 continue
cC ************* only read to LEXOG-3 because NB,pibar,NBbase are read
cC ************* from the data file, not the exogenous file.
c      DO 8020 I=1,LEXOG-3
...
!     >+((CBETABAR*CGAMMA)/(ONE+CBETABAR*CGAMMA))*VV(21,38,0)
     ...
     >+(Z(7)/(ONE+CBETABAR*CGAMMA))*VV(1,5,-1)
     >-((ONE+CBETABAR*CGAMMA*Z(7))/(ONE+CBETABAR*CGAMMA))*VV(1,5,0)
c     >+((CBETABAR*CGAMMA)/(ONE+CBETABAR*CGAMMA))*VV(21,38,0)
     >+((CBETABAR*CGAMMA)/(ONE+CBETABAR*CGAMMA))*PISTAR
     >+((ONE-Z(4))
     >*((ONE-CBETABAR*CGAMMA*Z(4))/((ONE+CBETABAR*CGAMMA)*Z(4)))
     >*(ONE/((CLANDAW-ONE)*CURVW+ONE)))
...
!     >-(CBETABAR*CGAMMA*VV(21,38,0)+Z(8)*VV(1,5,-1)))/
     >-(CBETABAR*CGAMMA*PISTAR+Z(8)*VV(1,5,-1)))/
